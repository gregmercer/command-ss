...
commandStanfordSpeak.prototype.beginsWith = function (search) {
  ...
  for (var prop in ss_data) {
    ...
    if (phrase.indexOf(search) == 0) {
      search_results[search_results.length] = { text: item.phrase + '\n' + item.definition };
    }
  }
...
commandStanfordSpeak.prototype.commandHelp = function (keyword) {
  ...
  var text = '*/ss begins with [search-term]*' + '\n';
  text += 'Searches for phrases that begin with the search term provided.' + '\n';
  text += 'Examples:' + '\n';
  text += '/ss begins with t' + '\n';
  text += '/ss begins with mem';

  help_text[help_text.length] = { text: text, color: 'good', 'username': 'stanford speaks', 'mrkdwn': 'true' };

  text = '*/ss keyword [search-term]*' + '\n';
  ...
  help_text[help_text.length] = { text: text, color: 'good', 'mrkdwn': 'true', username: 'stanford speaks' };
